...
Files missing from the original inventory, now classified below: `services/videoRecommendations.js`, `services/search.js`, `services/dataTransfer.js`, and components `StudyModeContent.jsx`, `SubjectNotes.jsx`, `PodcastPlayer.jsx`, `LibrarySearch.jsx`, `SubjectExam.jsx`, `SubjectStudio.jsx`, `LessonPlayer.jsx`, `SmartImageSheet.jsx`, `VideoRecommendations.jsx`, `StudyPlan.jsx` (all part of the Subject Studio feature — already existed when the plan was first drafted, just weren't counted).

## Migration scope, by effort tier

### Tier 0 — Reuses unchanged (no rewrite)

- **`api/` + `api/_lib/`** — Azure OpenAI / MiniMax provider abstraction, prompts, TTS/STT/vision/video-recommendations endpoints, auth. Plain Node/Vercel serverless code, no DOM dependency. The RN app calls the exact same HTTP endpoints.
- **`services/demoResponses.js`, `services/subjectStudy.js`, `services/videoRecommendations.js`, `services/search.js`** — pure data/fetch/string logic. Their only `window.*` calls are `setTimeout`/`clearTimeout`, which are global in React Native too.
...
**Decision made:** the RN app will not reconstruct video frame-by-frame at all. The generated opening clip was later removed; `LessonPlayer.jsx` is now a narrated slide lesson with a title card. Concretely:
