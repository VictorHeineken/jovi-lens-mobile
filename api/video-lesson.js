...
export default async function handler(req, res) {
  if (req.method === 'GET') {
    ...
    const getSession = sessionUser(req);
    if (getSession.provided && !getSession.user) return res.status(401).json({ code: 'SESSION_INVALID', message: 'Sessão expirada. Faça login novamente.' });
