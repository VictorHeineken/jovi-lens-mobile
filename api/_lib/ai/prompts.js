...
export function learningPreferenceContext(preferences = {}) {
  ...
  const CALENDAR_LABEL = { google: 'Google Agenda', outlook: 'Outlook' };
  ...
  const calendarLine = events.length
    ? ` Calendário conectado (${CALENDAR_LABEL[preferences.studyCalendar.provider] || 'Google Agenda'}): ${events.map((event) => `${event.title} em ${event.startsAt}${event.topics?.length ? `; tópicos: ${event.topics.join(', ')}` : ''}`).join(' | ')}. Use essas datas para definir urgência real por matéria.`
    : '';
...
{"subject":"${ctx.name}","overview":"1-2 frases de estratégia","calendarEvent":{"title":"nome da prova","startsAt":"data ISO","source":"Google Agenda","topics":["tópico"],"strategy":"como o plano muda por causa da data"},"sessions":[{"label":"Dia 1","focus":"subtema/tema","durationMinutes":30,"tasks":["tarefa concreta"]}],"spacedReview":[{"topic":"subtema","when":"em 1 dia|em 3 dias|em 1 semana"}]}
...
export function buildPodcastScriptPrompt(subject, { format = 'dialogue', preferences = {} } = {}) {
  ...
  if (format === 'drive') {
    return `${SUBJECT_PERSONA} Escreva um episódio de podcast para ouvir no carro sobre a matéria "${ctx.name}". O aluno não deve precisar escrever. O formato é um bate-papo guiado com IA: a IA faz perguntas curtas, pede para o aluno responder em voz alta, faz uma pausa indicada no texto e depois dá feedback do que estava certo ou faltando. Use linguagem falada, transições claras entre blocos, exemplos simples, pequenas retomadas e frases com cadência humana. ${preferenceLine} Não peça atividades que exigem escrever ou usar o celular enquanto dirige. Para "segments", use speaker "coach" nas perguntas/comandos da IA e "feedback" nas correções. Em "interactions", crie as mesmas perguntas em formato de toque para quando o aluno estiver parado. Formato exato:
{"subject":"${ctx.name}","format":"drive","title":"título do episódio","durationMinutes":12,"takeaways":["ideia que o aluno deve lembrar"],"segments":[{"speaker":"coach","text":"pergunta natural e pedido para responder em voz alta"},{"speaker":"feedback","text":"correção natural depois da pausa"}],"interactions":[{"id":"slug-curto","topic":"subtema","prompt":"pergunta para o aluno","options":[{"id":"a","text":"alternativa","correct":true},{"id":"b","text":"alternativa","correct":false}],"feedbackCorrect":"feedback para acerto","feedbackWrong":"feedback para erro"}]}
